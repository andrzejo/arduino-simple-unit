...
<xsl:output method="xml" indent="yes"/>
	<xsl:template match="/Site">
		<xsl:variable name="StartTestTime"><xsl:value-of select="Testing/StartTestTime/text()"/></xsl:variable>
		<testsuite timestamp="{$StartTestTime}">
    ...
    <xsl:template match="Testing/Test">
		...
		<xsl:variable name="exectime">
            ...
            <xsl:if test="@Status = 'failed'">
                ...
                <error message="{$failtype} ({$failcode})"><xsl:value-of select="replace(Results/Measurement/Value/text(), '(\[\d+m)|(\[NON-XML-CHAR-.+?\])', '')" /></error>
            </xsl:if>
            <xsl:if test="@Status = 'notrun'">
                <skipped><xsl:value-of select="replace(Results/Measurement/Value/text(), '(\[\d+m)|(\[NON-XML-CHAR-.+?\])', '')" /></skipped>
            </xsl:if>
	    <system-out><xsl:value-of select="replace(Results/Measurement/Value/text(), '(\[\d+m)|(\[NON-XML-CHAR-.+?\])', '')" /></system-out>
        </testcase>
    </xsl:template>
